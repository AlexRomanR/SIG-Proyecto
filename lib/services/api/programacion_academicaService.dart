...
import 'package:gestion_asistencia_docente/models/programacion_academica.dart';
...
class ProgramacionAcademicaService extends ChangeNotifier {
  ...
  Future<List<ProgramacionAcademica>> loadProgramacionAcademica(String userId, String token) async {
    isLoading = true;
    progAcademica = [];

    final resp = await http.get(
      Uri.parse('${servidor.baseURL}adminuser/get-all-docenteMateriasByUser/$userId'),
      headers: {
    ...
    if (resp.statusCode == 200) {
      final List<dynamic> progAcademicaMap = json.decode(utf8.decode(resp.bodyBytes));

      progAcademica = progAcademicaMap.map((element) => ProgramacionAcademica.fromMap(element)).toList();
...
bool existeAsistencia(String docenteMateriaId, DateTime fechaActual) {
  ...
  print('Fecha Actual: ${fechaActual.day}/${fechaActual.month}/${fechaActual.year}');

  bool exists = asistencias.any((asistencia) =>
    asistencia.docenteMaterias.id == docenteMateriaId &&
    asistencia.fecha == '${fechaActual.day}/${fechaActual.month}/${fechaActual.year}'
  );
